...
def refresh_data(years: int = 10, force: bool = False) -> dict[str, Any]:
    ...
    try:
        dca.refresh_data(force=force)
    except Exception as exc:  # noqa: BLE001 - a DCA refresh failure must not break the main flow
        print(f"[dca] refresh skipped: {exc}")
...
def main() -> None:
    ...
    if args.fetch_only:
        return
    if args.dca_check:
        dca.print_dca(dca.build_dca_decision())
        return
