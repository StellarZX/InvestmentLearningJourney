...
# 打包成 exe 后：BASE = exe 所在目录；脚本运行时 = 脚本所在目录
if getattr(sys, 'frozen', False):
    BASE = os.path.dirname(sys.executable)
    _cands = [os.path.join(BASE, 'data', 'portfolio.db'),                       # exe 旁 data/
              os.path.join(os.path.dirname(BASE), 'data', 'portfolio.db'),      # 项目根 data/
...
form#editForm{display:grid;grid-template-columns:repeat(8,auto) 1fr auto;gap:8px;align-items:end;margin-top:10px}
...
<div class="kpi"><div class="k">余额宝余额</div><div class="v" id="kCash">-</div></div>
<div class="kpi"><div class="k">余额宝推算余额</div><div class="v" id="kCashImplied">-</div><div class="hint" style="margin-top:2px">扣已买基金</div></div>
...
<div><label>收益(卖)</label><input type="number" id="fGain" step="0.01" style="width:80px" value="0"></div>
...
<table><thead><tr><th>ID</th><th>日期</th><th>类别</th><th>基金</th><th>代码</th><th>方向</th><th>金额</th><th>收益</th><th>备注</th><th>操作</th></tr></thead>
...
function loadSummary(){
  fetch('/api/summary').then(r=>r.json()).then(s=>{
    ...
    document.getElementById('kCash').textContent='¥'+s.summary['资金池'].total.toLocaleString();
...
function edit(id){
  fetch('/api/trans?id='+id).then(r=>r.json()).then(t=>{
    ...
    document.getElementById('fGain').value=t.gain||0;
...
function resetForm(){
  ...
  buildFundField('指数');
  document.getElementById('fAmt').value=''; document.getElementById('fGain').value='0'; document.getElementById('fNote').value='';
...
function save(){
  ...
  const p={ date:document.getElementById('fDate').value.trim(),
    ...
    gain:document.getElementById('fGain').value||0,
